...
if($sunday_first == 1)	$days = array("$lang_sunday", "$lang_monday", "$lang_tuesday", "$lang_wednesday", "$lang_thursday", "$lang_friday", "$lang_saturday");

$months = array("","01","02","03","04","05","06","07","08","09","10","11","12");
if ($use_lang_var == "1")	$months = array("", "$lang_january", "$lang_february", "$lang_march", "$lang_april", "$lang_may", "$lang_june", "$lang_july", "$lang_august", "$lang_september", "$lang_october", "$lang_november", "$lang_december");

// Don't use calendar
if($cfgrow['calendar'] == "No Calendar")	$tpl = ereg_replace("<SITE_CALENDAR>"," ",$tpl);
...
if($cfgrow['calendar'] == "Normal")
{
	...
	if(date("L", mktime(0,0,0,$curr_month,1,$curr_year)))	$total_days[2] = 29;
	...
	while($day_count <= $total_days[$curr_month])
	{
		...
		if($day_count <= date("j") && $curr_year == date("Y") && $curr_month == date("n") OR ($curr_month < date("m") AND $curr_year <= date("Y")))	$class = "td-calendar-days-vz";
		else	$class = "td-calendar-days-vz";
...
if($cfgrow['calendar'] == "Horizontal")
{
	...
	while($day_count <= $total_days[$curr_month])
	{
		...
		if($day_count <= date("j") && $curr_year == date("Y") && $curr_month == date("n") OR ($curr_month < date("m") AND $curr_year <= date("Y")))	$class = "td-calendar-days";
		else	$class = "td-calendar-days";
