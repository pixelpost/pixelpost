...
if (!isset($_GET['view']) or $_GET['view'] == '')
{
    ...
    if (isset($_GET['x']) and $_GET['x'] == "save")
    {
        ...
        if ($_POST['autodate'] == 1)
        {
            ...
            if ($row)
                $datetime = $row[0]; // If there is none, will default to the other value
        } else
            if ($_POST['autodate'] == 2)
            {
...
?>
   ...
   <input type="text" name="headline" style="width:550px;" value="<?php if (isset($status) && $status != 'ok')
            echo $_POST['headline']; ?>"/><p/>
	<?php echo $admin_lang_ni_tags; ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
   <input type="text" name="tags" style="width:550px;" value="<?php if (isset($status) && $status != 'ok')
            echo $_POST['tags']; ?>"/><br/>
...
?>
    ...
    <textarea name="body" style="width:97%;height:100px;" rows="" cols=""><?php if (isset($status) && $status != 'ok')
            echo $_POST['body']; ?></textarea><p/>
	</div>
    </div>
    <div class='jcaption'>
    <?php echo $admin_lang_ni_datetime; ?></div>
    <div class='content'>
     <input type="radio" name="autodate" value="2" <?php if ($selected_autodate[2])
            echo 'checked="checked" '; ?>id="postnow"/><label for="postnow"><?php echo $admin_lang_ni_post_now . ' (~' . $cur_time . ')'; ?></label><br/>
     <input type="radio" name="autodate" value="1" <?php if ($selected_autodate[1])
            echo 'checked="checked" '; ?>id="postdayaft"/><label for="postdayaft"><?php if ($cfgrow['daysafterlastpost'] == 1)
        {
     ...
     <input type='radio' name='autodate' value='3' <?php if ($selected_autodate[3])
            echo 'checked="checked" '; ?>id="exifdate"/><label for="exifdate"><?php echo $admin_lang_ni_post_exif_date; ?></label><br/>
     <input type='radio' name='autodate' value='0' <?php if ($selected_autodate[0])
            echo 'checked="checked" '; ?>id="specificdate"/><label for="specificdate"><?php echo $admin_lang_ni_post_spec_date; ?></label><br/><br/>
